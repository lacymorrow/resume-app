...
export function FilterPanel({ filters, allTags, workMatches, projectMatches, totalWork, totalProjects, onFiltersChange, onExport, onClose }: FilterPanelProps) {
  ...
  const setFlavor = (flavorId: string) => {
    ...
    if (!flavor) return;
    onFiltersChange({ ...filters, flavorId, sections: flavor.sections, selectedTags: [] });
  };
  ...
  const activeFlavor = FLAVORS.find((f) => f.id === filters.flavorId);
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between px-4 pt-4 pb-2">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setFlavor("complete")}>
        ...
        <div>
          ...
          {activeFlavor && activeFlavor.id !== "complete" && (
            <p className="mt-1.5 text-xs text-muted-foreground italic">
              Showing as: {activeFlavor.tagline}
            </p>
          )}
        </div>
